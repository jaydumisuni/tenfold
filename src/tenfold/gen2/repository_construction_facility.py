...
def _function_defaults_snapshot(func) -> tuple:
    ...
    if not all(type(name) is str for name in kwdefaults):
        ...
        # this same file) exists yet -- a plain `TypeError` is used
        # here instead, matching the fact that this specific branch is
        # pure defense-in-depth for a genuinely-defined function's
        # OWN, freshly-created `__kwdefaults__` (whose keys are always
        # `str`, generated by Python itself from real parameter names)
        # rather than a live-admission rejection.
...
def _function_defaults_match(func, captured_snapshot: tuple) -> bool:
    ...
    its own SELF-CAUGHT SORT-BEFORE-TYPE-CHECK FINDING section --
    THIS function is the one the reviewer's reproduction actually
    targeted (it runs on `func.__kwdefaults__` AFTER admission, the
    genuinely attacker-reachable side of this pair, unlike
