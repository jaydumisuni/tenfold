...
// ============================================================================
// G2-23: Effect authority-slice migration (G2-00 SS15-16). Reuses
// `identity_generation`'s generic, artifact-identity-parameterized
// admission wrappers exactly as `rust/dispatch_lease` does for its own
// two slices -- `admit_transition_for`'s expected_from_ref/
// expected_to_ref binding (added after G2-23 part 1's round-2 review)
// means the named wrappers below reject any record not genuinely bound
// to this slice's own refs.
// ============================================================================

pub const EFFECT_CENSUS_TRANSFER_FROM_REF: &str = "gen1-effect-census";
pub const EFFECT_CENSUS_TRANSFER_TO_REF: &str = "gen2-effect-census";

pub fn effect_census_transfer_trust_table_row() -> trust_table::TrustTableRow {
    trust_table::TrustTableRow {
        artifact_identity: "effect_census_transfer".into(),
        independently_checks: vec![
...
pub fn admit_check_effect_census_transfer_transition(
    table: &trust_table::TrustTable,
    current: identity_generation::AuthorityTransferStage,
    new_stage: identity_generation::AuthorityTransferStage,
) -> Result<(), EffectCensusError> {
    identity_generation::admit_check_authority_transfer_transition_for(table, "effect_census_transfer", current, new_stage).map_err(|e| err(e.to_string()))
}

pub fn admit_effect_census_transfer_transition(
    table: &trust_table::TrustTable,
    record: &identity_generation::AuthorityTransferRecord,
    new_stage: identity_generation::AuthorityTransferStage,
    policy: &identity_generation::AuthorityTransferStabilizationPolicy,
) -> Result<identity_generation::AuthorityTransferRecord, EffectCensusError> {
    identity_generation::admit_transition_for(
        table,
        "effect_census_transfer",
        EFFECT_CENSUS_TRANSFER_FROM_REF,
        EFFECT_CENSUS_TRANSFER_TO_REF,
        record,
        new_stage,
        policy,
    )
...
mod tests {
    ...
    // ---- G2-23: Effect authority-slice migration ----

    fn admitted_effect_transfer_table() -> trust_table::TrustTable {
        let mut table = trust_table::initial_trust_table();
        table.extend(trust_table_row()).unwrap();
        table.extend(effect_census_transfer_trust_table_row()).unwrap();
        table
    }
    ...
    fn stabilizing_effect_transfer_record() -> identity_generation::AuthorityTransferRecord {
        identity_generation::AuthorityTransferRecord {
            transfer_id: "effect-census-transfer-X-1".into(),
            from_authority_ref: EFFECT_CENSUS_TRANSFER_FROM_REF.into(),
            to_authority_ref: EFFECT_CENSUS_TRANSFER_TO_REF.into(),
    ...
    #[test]
    fn effect_census_transfer_row_is_well_formed() {
        assert!(effect_census_transfer_trust_table_row().is_well_formed());
    }

    #[test]
    fn admit_check_effect_census_transfer_transition_fails_closed_when_table_has_no_row() {
        let table = trust_table::TrustTable::new();
        assert!(admit_check_effect_census_transfer_transition(&table, identity_generation::AuthorityTransferStage::PREPARED, identity_generation::AuthorityTransferStage::STAGED).is_err());
    }

    #[test]
    fn admit_check_effect_census_transfer_transition_succeeds_once_admitted() {
        admit_check_effect_census_transfer_transition(&admitted_effect_transfer_table(), identity_generation::AuthorityTransferStage::PREPARED, identity_generation::AuthorityTransferStage::STAGED)
            .expect("legal transition on an admitted table should succeed");
    }

    #[test]
    fn admit_effect_census_transfer_transition_succeeds_once_admitted_with_full_evidence() {
        let record = identity_generation::AuthorityTransferRecord { stabilization_evidence: full_transfer_evidence(), ..stabilizing_effect_transfer_record() };
        let result = admit_effect_census_transfer_transition(&admitted_effect_transfer_table(), &record, identity_generation::AuthorityTransferStage::STABILIZATION_PROVEN, &full_transfer_policy());
        assert!(result.is_ok());
        assert_eq!(result.unwrap().stage, identity_generation::AuthorityTransferStage::STABILIZATION_PROVEN);
    }

    #[test]
    fn admit_effect_census_transfer_transition_rejects_incomplete_evidence_even_when_admitted() {
        let result = admit_effect_census_transfer_transition(&admitted_effect_transfer_table(), &stabilizing_effect_transfer_record(), identity_generation::AuthorityTransferStage::STABILIZATION_PROVEN, &full_transfer_policy());
        assert!(result.is_err());
    }

    #[test]
    fn admit_effect_census_transfer_transition_rejects_a_record_bound_to_a_foreign_slice() {
        // Round-2 G2-23 part 1 review lesson, applied proactively here:
        // a record with the wrong from/to refs must never be admittable
        ...
        let record = identity_generation::AuthorityTransferRecord {
            ...
            ..stabilizing_effect_transfer_record()
        };
        let result = admit_effect_census_transfer_transition(&admitted_effect_transfer_table(), &record, identity_generation::AuthorityTransferStage::STABILIZATION_PROVEN, &full_transfer_policy());
        assert!(result.is_err());
    }
