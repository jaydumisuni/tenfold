...
//! - `high-risk-reach-admission` — reads an `EffectReachResult` JSON object from stdin, prints ACCEPT/ERROR.
//! - `observation-cover-containment` — reads `{"authorized_mutation_domain": [String], "effect_reach": EffectReachResult, "observation_cover": ObservationCover}` from stdin, prints ACCEPT/ERROR.
...
use capability_graph::{
    ...
    CapabilityCausationGraph, ContainingScopeTraversalResult, EffectReachResult, EffectivePolicyClaim, ObservationCover,
};
...
fn main() -> ExitCode {
    ...
    match command.as_str() {
        ...
        "high-risk-reach-admission" => {
            ...
            let result: EffectReachResult = match serde_json::from_str(&buf) {
                Ok(v) => v,
                Err(e) => {
                    println!("ERROR: {e}");
                    return ExitCode::from(1);
            ...
            match admit_check_high_risk_reach_admission(&admitted_table(), &result) {
                Ok(()) => {
                    println!("ACCEPT");
                    ExitCode::SUCCESS
                }
        ...
        "observation-cover-containment" => {
            ...
            match admit_check_observation_cover_containment(&admitted_table(), &request.authorized_mutation_domain, &request.effect_reach, &request.observation_cover) {
                Ok(()) => {
                    println!("ACCEPT");
                    ExitCode::SUCCESS
                }
